...
namespace Xceed.Words.NET
{
    internal static class XElementHelpers
    {
        ...
        internal static T GetValueToEnum<T>(XElement element)
        {
            ...
            foreach (T e in Enum.GetValues(typeof(T)))
            {
                ...
                if (fi.GetCustomAttributes(typeof(XmlNameAttribute), false).Any())
                    throw new NullReferenceException(String.Format("Attribute 'XmlNameAttribute' is not assigned to {0} fields!", typeof(T).Name));
        ...
        internal static String GetXmlNameFromEnum<T>(T value)
        {
            ...
            if (fi.GetCustomAttributes(typeof(XmlNameAttribute), false).Any())
                throw new NullReferenceException("Attribute 'XmlNameAttribute' is not assigned to {0} fields!");
            var a = (XmlNameAttribute)fi.GetCustomAttributes(typeof(XmlNameAttribute), false).First();
            return a.XmlName;
        }
    }
