...
namespace Xceed.Words.NET
{
    internal static class HelperFunctions
    {
        ...
        internal static void RenumberIDs(DocX document)
        {
            ...
            int totalTrackerIds = trackerIDs.Count();
            for (int i = 0; i < totalTrackerIds; i++)
        ...
        internal static Paragraph GetFirstParagraphEffectedByInsert(DocX document, int index)
        {
            ...
            if (document._paragraphLookup.Keys.Any() && index == 0)
                return null;
        ...
        internal static bool ContainsEveryChildOf(XElement elementWanted, XElement elementToValidate, MatchFormattingOptions formattingOptions)
        {
            ...
            {
                if (!elementToValidate.Elements(subElement.Name).Any(bElement =>
                bElement.GetAttribute(XName.Get("val", DocX.w.NamespaceName)) == subElement.GetAttribute(XName.Get("val", DocX.w.NamespaceName))))
                    return false;
            }
